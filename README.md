...
> 「侵入性」是指强迫你使用新的语法、数据类型。比如 B**kb**e，为了实现 MVVM 的效果，你需要创建各种 Model、View 实例，表面上你是在写 JS，实际上你是在写基于那个库的 API 的某种方言（似曾相识：你以为你在写 JS，其实你写的是 jQuery）。反观 Vue，你只是在给某个对象的某个属性赋值而已，用以实现 MVVM 的逻辑并没有体现在你书写的代码上。这就是「非侵入式」。
